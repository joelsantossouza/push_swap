/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   stack_insertion_align.c                            :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: joesanto <joesanto@student.42porto.com>    +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/12/04 09:26:07 by joesanto          #+#    #+#             */
/*   Updated: 2025/12/04 13:02:50 by joesanto         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "push_swap.h"

void	stack_insertion_align(size_t idx_a, t_stack *stack_a, size_t idx_b, t_stack *stack_b)
{
	const int		a_half = idx_a <= (stack_a->size >> 1);
	const int		b_half = idx_b <= (stack_b->size >> 1);
	size_t			min_cost;
	size_t			new_cost;
	align_to_top	align;

	if (a_half && b_half)
		return (sync_rotate_up(idx_a, stack_a, idx_b, stack_b));
	if (!a_half && !b_half)
		return (sync_rotate_down(idx_a, stack_a, idx_b, stack_b));
	align = sync_rotate_up;
	min_cost = sync_rotate_up_cost(idx_a, stack_a->size, idx_b, stack_b->size);
	new_cost = sync_rotate_down_cost(idx_a, stack_a->size, idx_b, stack_b->size);
	if (new_cost < min_cost)
	{
		align = sync_rotate_down;
		min_cost = new_cost;
	}
	new_cost = rotate_to_top_cost(idx_a, stack_a->size, idx_b, stack_b->size);
	if (new_cost < min_cost)
	{
		align = rotate_to_top;
		min_cost = new_cost;
	}
	align(idx_a, stack_a, idx_b, stack_b);
}
